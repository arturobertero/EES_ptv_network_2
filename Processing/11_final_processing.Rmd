...
p_load(tidyverse, here, haven, countrycode, vdemdata, psych, lavaan, ltm,
       ...
       hrbrthemes, viridis, ggrepel, jtools, patchwork)
...
## Summary and graph
We have two solid predictors: p_inst and pola. Pint is not significant and, additionally, points in the "wrong" direction. Now the scatterplot between huge_aspl and p_inst. Unfortunately, it looks miserable. Correlation between the two measures is -0.01206442.The scatterplot is updated (corrected labels).
...
Alternative plot 1: scatterplots with facet wrap by country. Here we see the problems might be related to the very low variance of party_institutionalization.
...
Alternative plot 2: scatterplots with facet wrap by year. Here we see the problems might be related to the very low variance of party_institutionalization. It is still very hard to grasp a negative relationship between the two. 
...
Alternative plot 3: same with labels.
```{r}
# Create the scatterplot with facet wrap by country
scatt_faceted_3 = ggplot(data_scatt_2, aes(x = p_inst, y = EES_GGM_huge_aspl)) +
  geom_point(size = 0.5) + 
  geom_smooth(method = "lm", color = "pink", fill = "#69b3a2", se = TRUE) +
  labs(x = "Party institutionalization",
       y = "ASPL") +
...
# Print the faceted scatterplot
print(scatt_faceted_3)
```

Alternative plot 4: effect_plot. Here we have a slope for p_inst and pola. We are plotting the predicted regression line of the full model (country + ye F.E.). 

```{r}
# Generate the effect plots
eff_1 = effect_plot(huge_aspl_3_c, pred = p_inst, interval = TRUE, plot.points = TRUE) +
  labs(x = "Party institutionalization", y = "ASPL") +
  theme(plot.margin = margin(0, 0, 0, 0, "pt")) +
  theme_ipsum()

eff_2 = effect_plot(huge_aspl_3_c, pred = pola, interval = TRUE, plot.points = TRUE) +
  labs(x = "Party polarization", y = "ASPL") +
  theme(plot.margin = margin(0, 0, 0, 0, "pt")) +
  theme_ipsum()

eff_3 = effect_plot(huge_aspl_3_c, pred = pint, interval = TRUE, plot.points = TRUE) +
...
plot_coef <- plot_summs(huge_aspl_3, huge_aspl_3_c, coefs = coef_select, colors = c("pink", "#69b3a2"), 
...
Once again we have solid findings for the weighted measures, even if considering turnout. I also add ASPL as covariate, but it is never significant.   
...
tab_model(list(tur_eu_i_c, tur_eu_w_c, tur_na_i_c, tur_na_w_c),
          ...
          string.pred = " ",
          auto.label = FALSE,
          terms = c("EES_GGM_huge_np_i", "EES_GGM_huge_np_w", "mand", "gdp", "infla", "ihdi", "pola", "EES_GGM_huge_aspl"))
...
A closer inspection of the variable mandatory voting. It was previously coding 1 countries with STRICT legislation for mandatory voting. It is now scoring 1 also for countries with WEAK legislationfor mandatory voting (this is the coding of the IDEA database). 
