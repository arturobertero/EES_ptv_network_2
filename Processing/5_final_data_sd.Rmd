...
```{r}
# Compute PTV_SD (Standard Deviation of PTV Scores) and PTV_M (Mean of PTV Scores)
data <- data %>%
  rowwise() %>%
  mutate(
    PTV_SD = ifelse(n_PTV > 1, sd(c_across(starts_with("t_var_ptv")), na.rm = TRUE), NA_real_),
    PTV_M  = ifelse(n_PTV > 0, mean(c_across(starts_with("t_var_ptv")), na.rm = TRUE), NA_real_)
  ) %>%
  ungroup()
