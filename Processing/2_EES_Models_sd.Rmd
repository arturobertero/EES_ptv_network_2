...
```{r}
# Function to compute DIFF_SD (standard deviation of absolute differences)
compute_diff_sd <- function(difference_matrix) {
  if (!is.null(difference_matrix) && nrow(difference_matrix) > 0) {
    return(sd(difference_matrix$Difference, na.rm = TRUE))  # Compute SD of absolute differences
  } else {
    return(NA_real_)  # Return NA if no valid data
  }
}

# Function to add DIFF_SD to each dataset in EES
add_diff_sd <- function(country_data, country_name, year) {
  country_data %>%
    mutate(DIFF_SD = map_dbl(EES_mat[[year]][[country_name]][["Difference_Matrix"]], compute_diff_sd))
}

# Apply the function across all year-country datasets
EES <- map(names(EES), function(year) {
  map(names(EES[[year]]), function(country_name) {
    add_diff_sd(EES[[year]][[country_name]], country_name, year)
  }) %>% set_names(names(EES[[year]]))  # Preserve country names
}) %>% set_names(names(EES))  # Preserve year names
