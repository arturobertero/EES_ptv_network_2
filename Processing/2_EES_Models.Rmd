...
compute_difference_matrix <- function(ptv_row) {
  ptv_values <- as.numeric(ptv_row)  # Convert to numeric
  party_labels <- names(ptv_row)  # Extract column names
  
  # Ensure that we have valid party labels
  if (length(ptv_values) < 2) {
    return(NULL)  # If fewer than 2 parties, return NULL (to prevent errors)
  }
...
generate_difference_matrices <- function(country_data) {
  country_data %>%
    ...
    rowwise() %>%
    mutate(Difference_Matrix = list(compute_difference_matrix(cur_data()))) %>%  # Use cur_data() to keep names
    ungroup() %>%
...
compute_mean_constraint <- function(difference_matrix) {
  if (!is.null(difference_matrix)) {
    return(mean(difference_matrix$Difference, na.rm = TRUE)) # Compute mean absolute difference
  } else {
    return(NA_real_)  # Return NA if no difference matrix is available
  }
}
