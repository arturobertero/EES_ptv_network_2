...
for(year in names(EES)) {
  ...
  for(country_code in names(EES[[year]])) {
    tryCatch({
      ...
      network_estimate = estimateNetwork(country_data, default = "EBICglasso", tuning = 0.25)
...
for(year in names(EES_GGM_glasso)) {
  ...
  for(country_code in names(EES_GGM_glasso[[year]])) {
    ...
    EES_GGM_glasso[[year]][[country_code]] = list(graph = graph_object)
  }
}
...
for(year in names(EES)) {
  ...
  for(country_code in names(EES[[year]])) {
    tryCatch({
      # Attempt to process the data
      country_data = EES[[year]][[country_code]]
      network_estimate = estimateNetwork(country_data, default = "huge", tuning = 0.25)
...
for(year in names(EES_GGM_huge)) {
  ...
  for(country_code in names(EES_GGM_huge[[year]])) {
    ...
    # Assemble the final list with 'graph' as a data frame
    EES_GGM_huge[[year]][[country_code]] = list(graph = graph_object)
  }
}
...
```{r}
# make _huge list equal to _COR list
...
for (year in names(EES_GGM_huge)) {
  ...
  for (country_code in names(EES_GGM_huge[[year]])) {
    ...
    graph <- EES_GGM_huge[[year]][[country_code]][["graph"]]
    
    # Assign the graph object to the upper level of the list
    EES_GGM_huge[[year]][[country_code]] <- graph
  }
}
...
```{r}
# make _glasso list equal to _COR list
...
for (year in names(EES_GGM_glasso)) {
  ...
  for (country_code in names(EES_GGM_glasso[[year]])) {
    ...
    # Extract the graph object
    graph <- EES_GGM_glasso[[year]][[country_code]][["graph"]]
    
    # Assign the graph object to the upper level of the list
    EES_GGM_glasso[[year]][[country_code]] <- graph
  }
}
